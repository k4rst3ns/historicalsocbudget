...
BB: braket includes sandyness instead of water.
I would leave out the braket, and just state that non-crop areas are neither tilled nor irrigated. then the formulas are still correct.
...
We calculate SOC stocks based on the area shares of land use types (lut) within the half-degree grid cells (i). If land is converted from one land use type (lut) into another(!lut) (BB: like this? then you need to change the order of the !lut in the equation; lut is reduced, !lut is expanded), a respective share of the SOC stocks have to be reallocated as well. We account for land conversion at the beginning of each time step $t$ by calculating a preliminary stock $SOC_{lut,t*}$ via 
...
with $A$ being the area, $AR$ the area reduction and $AE$ the area expansion for a given land use type $lut$. Note that $!lut$ denotes the (sum over) (BB: why braket?) land use type(s), which decrease(s) in the specific time step $t$. Data sources and methodology on land use states and changes are descripted in \@ref(sec:landuse).    
