...
SOCPointValid  <- ggplot() + 
                  geom_point(data = statsPoints, 
                            aes(alpha = str_wrap("Median (R2=0.13)", 7), x = median, y = thisStudy, colour = factor(Native)),
                            shape = 3, size = 2) +
                  geom_segment(data = statsPoints,
                               aes(alpha = "Min-Max", x = min, y = thisStudy, xend = max, yend = thisStudy, 
                                   colour = factor(Native)), 
                               size = 0.5) +
                  geom_point(data = allPoints, 
                             aes(alpha = "Point Data", x = ValidationData, y = thisStudy, colour = factor(Native)),
                             shape = 1, size = 0.5) +
                  ...
                  scale_color_manual(values = scen_cols, labels = c("Cropland", str_wrap("Natural vegetation", 7)), name = "") +
...
ggsave(paste0(outputdir,"scatterComparePointData_2010.png"), plot=SOCPointValid, height=16, width=16, units="cm", dpi = 300)
