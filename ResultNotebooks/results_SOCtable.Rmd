...
scenarios <- c("rev30histManagement") #,"rev18histManagement_ResAllom_2021_02_01")
outputdir <- "Output/"
...
years             <- findset("past")
climateIPCCr      <- readRDS("Data/rev30histManagement/IPCC_reduced.rds")
...
# CShare_ipcczone   <- dimSums(climateIPCCr * SOCDens[,,"crop"][,,"actualstate"]   * Landuse[,years,"crop"], dim=1)/
#                     dimSums(climateIPCCr * SOCDens[,,"natveg"][,,"naturalstate"] * Landuse[,years,"crop"], dim=1)

CShare_ipcczone   <- dimSums(climateIPCCr * toolConditionalReplace(SOCDens[,,"crop"][,,"actualstate"]/SOCDens[,,"natveg"][,,"naturalstate"],
                                                                  c("is.na()", "is.nan()", "is.infinite()",">3"), c(1,1,1,5)) * Landuse[,years,"crop"], dim=1) /
                     dimSums(climateIPCCr * Landuse[,years,"crop"], dim=1)

getNames(CShare_ipcczone) <- gsub("_", " ", getNames(CShare_ipcczone))
getCells(CShare_ipcczone) <- "GLO" 
...
climateIPCCr       <- readRDS("Data/rev30histManagement/IPCC_reduced.rds")
climateIPCC        <- readRDS("Data/rev30histManagement/IPCC.rds")
...
comparison <- mbind(CShare_ipcc, 
                    round(add_dimension(setYears(collapseNames(CShare_ipcczone[,"y1975","naturalstate"]),NULL),
                                  nm="This Study.hist.1975",add="model.input.year"),2),
