...
#    untar(paste0("Data/", paste0(rev,item), "_h12_carbonbudget.tgz"),
#    exdir=paste0("Data/", paste0(rev,item)), extras="--keep-newer-files ") # files=filesNeeded,
# }
...
cropland_share <- dimSums(SoilCarbon[,,"actualstate"][,,"crop"], dim=3)/(dimSums(SoilCarbon[,,"naturalstate"][,,"natveg"], dim=3) + 1e-8)
...
SOCemis <- ggplot() + geom_line(data= as.data.frame(-glo_SOCEmission)[,c(3,6)],  aes(x = Year, y = Value, group=1)) +
  ...
  annotate("segment", x = factor(1990), xend = factor(1990), y = 0, yend = -36, arrow=arrow(end="both", type="closed", length=unit(0.1, "inches")))
...
SOCplotmap   <- function(data, name, cols, limits, values = NULL, breaks, labels) {
  ...
  out <- ggplot(data = world) + geom_sf() + 
    geom_tile(data = subset(data, Year == "2010", c("lon","lat","Value")), aes(x = lon, y = lat, fill = Value)) +
...
SOCshare    <- SOCplotmap( data     = share2plot,
                            limits   = c(0,2),
                            cols     = cols,
...
SOCincrease <- SOCplotmap( data     = increase2plot,
                            limits   = c(-100, 100),
                            cols     = cols,
                            ...
                            labels   = c("< -40", "        -30","        -20","      -10 ","     0","    10","> 10"))


cols <- colorRampPalette(brewer.pal(9, "YlGn"))(8)[2:8]
SOCdensity  <- SOCplotmap( data     = density2plot,
                            limits   = c(0, 150), 
                            cols     = cols, 
...
grid <- plot_grid(SOCemis,SOCdensity,SOCincrease, SOCshare,  ncol=2, labels=c("(a)","(b)", "(c)", "(d)"),
