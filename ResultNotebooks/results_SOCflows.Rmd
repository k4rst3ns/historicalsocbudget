...
CropPRIMLoss      <- CropPRIM + ResPRIM + LivstCropHANPP - dimSums(grepSpaceTimeC(FAOmassbalance[,,c(kcr,kli,kres)][,,"other_util"]), dim=3) # only material use is not re-emitted
...
TotalSOCBudget    <- mbind(TotalSOCBudget,
                           setNames(SOCsub[,,"natveg"], gsub("\\.","_",getNames(SOCsub))),
                           SOCtot)

CarbonIn          <- grepSpaceTime(SteadyState[,,"alpha_in"]*Landuse)
