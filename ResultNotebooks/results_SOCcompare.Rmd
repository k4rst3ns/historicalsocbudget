...
# colnames(table)[1] <- "Model/Database"
# colnames(table)[6] <- "Global SOC in GtC"
...
glob <- ggplot(data = melt(subset(table, select =c(Data1, GLO, Year))), aes(x=Year,y=value, group=Data1)) + 
...
comp <- ggplot(data = melt(subset(table, select=-GLO)), aes(x=Year,y=value, group=Data1)) + 
  geom_point(aes(colour=Data1, size=Data1, shape=Data1)) + 
  facet_wrap(~ variable, scales="free", drop=TRUE, labeller = label_wrap_gen(width=8)) + 
  scale_size_manual(values=c(4,2,2,2,2,2)) + 
  scale_shape_manual(values=c(19,16,6,2,5,0)) +
  scale_color_manual(values = cols) +
  labs(color  = leg_name, size = leg_name, shape = leg_name) +
...
ggsave(paste0(outputdir,"/Images/glo_comparisonfigure.png"), plot=glob, height=8, width=8, units="cm", dpi = 300)
ggsave(paste0(outputdir,"/Images/reg_comparisonfigure.png"), plot=comp, height=10, width=14, units="cm", dpi = 300)
